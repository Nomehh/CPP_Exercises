...
struct MyObject
{
    bool operator<(const MyObject& obj) const { return attr_1 < obj.attr_1; }
...
#define ANSWER_01 false // on a pas de fct de hashage donc ca va pas compiler
...
#define ANSWER_10 true // invalide lui + tout ceux à sa droite
...
#define ANSWER_13 true // ca peut... (réalloc)
...
// Question 30
// La lambda `[](const std::string& v) { return v + v; }` a pour type std::function<std::string(const
// std::string&)>.
#define ANSWER_30 \
    false // faut faire gaffe, c'est un objet (un objet function, il a des attributs), elle a un type qu'on
          // connait pas, mais qu'on peut transf en function
...
#define ANSWER_33 false // esperluette à gauche
...
#define ANSWER_35 true // ca fait peur
...
#define ANSWER_41 false // C'est une ref universelle si Type est un vrai type
